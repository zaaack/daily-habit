      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[92vw] max-w-lg rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-2xl animate-scale-in focus:outline-none">
          <div className="flex items-start justify-between mb-3">
            <div>
              <Dialog.Title className="text-base font-semibold">同步冲突</Dialog.Title>
              <div className="text-xs text-slate-400 mt-0.5">{project?.emoji} {project?.name} · {items.length} 处分歧</div>
            </div>
              ...
              return (
                <div key={k} className="rounded-lg border border-slate-800 p-2.5 text-sm">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      <div className="text-[10px] uppercase text-slate-500 mb-0.5">本地</div>
                    ...
                    >
                      <div className="text-[10px] uppercase text-slate-500 mb-0.5">远端</div>
          ...
          <div className="flex items-center gap-2 mt-4">
            <button className="btn-ghost" onClick={() => void clear(projectId)}>稍后处理</button>
