...
export abstract class SqliteKyselyRepo implements Repo {
    ...
    protected async getDb(): Promise<Kysely<DB>> {
        if (!this.db)
            throw new Error("Repo not initialized. Call init() first.");
        return this.db;
    }
    ...
    protected async createTables(): Promise<void> {
        ...
        try {
            await db.schema
                .alterTable("projects")
                .addColumn("archived", "integer", (col) =>
                    col.notNull().defaultTo(0),
                )
                .execute();
        } catch {
    ...
    async upsertCheckin(c: Checkin): Promise<void> {
        ...
        await db
            ...
            .onConflict((cb) =>
                cb.columns(["project_id", "date"]).doUpdateSet({
                    status: c.status,
                    value: c.value,
                    note: c.note,
                    updated_at: c.updatedAt,
    ...
    async setKV(key: string, value: unknown): Promise<void> {
        ...
        await db
            .insertInto("settings_kv")
            .values({ key, value: serialized })
            .onConflict((cb) =>
                cb.column("key").doUpdateSet({ value: serialized }),
            )
            .execute();
    }
    ...
    async deleteKV(key: string): Promise<void> {
        const db = await this.getDb();
        await db.deleteFrom("settings_kv").where("key", "=", key).execute();
    }
}
